...
// SPIData structure with processing time (names kept as in your code)
class SPIData
{
public:
    SPIData() { init(); };
...
// --- Thread function: exact same polling loop moved here ---
void spi_thread_function()
{
    ...
    // Match your previous constants (adjust if you like)
    ...
    // Prepare initial reply and preload first byte
    ...
    printf("SPI Communication (polling in thread) started. Waiting for master...\r\n");
...
int main()
{
    // Launch the polling loop in its own thread (same logic as before)
    Thread spi_thread(osPriorityNormal);
    spi_thread.start(spi_thread_function);

    // Your main can do other work; keep it simple here
    while (true) {
